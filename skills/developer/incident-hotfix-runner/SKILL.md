...
5. Create the Bug in KDP with the fixVersion and links: **"Root Cause Fix" → the SSQ incident**, **relates/caused-by → the regressed story**. If the team parents hotfix items to an unplanned-work epic, follow that convention — the links carry the real trace either way.
