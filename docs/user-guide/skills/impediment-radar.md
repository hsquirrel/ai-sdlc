...
1. It scans for: explicitly **flagged** issues (Jira's Flagged field) with age; **silent stalls** (in-progress, no Jira/PR activity 2+ days, *unflagged* — the dangerous kind); **aging reviews** (PRs waiting 1+ day, or change requests gone quiet); **dependency rot** ("blocked by" links to other teams' issues that haven't moved, or resolved dependencies the dependent story never reacted to); and **recurrence** (re-flagged items — a process smell).
